...
async def query(request: QueryRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail="外部服务暂时不可用，请稍后重试")
    except TimeoutError as e:
        logger.error(f"查询超时: {e}")
        raise HTTPException(status_code=503, detail="查询超时，请稍后重试")
...
async def query_stream(request: QueryRequest):
    ...
    async def event_generator():
        ...
        try:
            ...
            # 获取流式生成器、来源信息和网络检索标记
            stream_gen, sources, used_web_search = await orchestrator.process_query_stream(
                question=request.question,
                use_web_search=use_web_search,
                top_k=top_k,
            )
